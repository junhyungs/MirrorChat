...
public class ChattingUI : NetworkBehaviour
{
    ...
    //        OnTestMessage(senderName, msg.Trim());
    //    }
    //}
    [Command(requiresAuthority = false)]
    void CommandSendMessage(string message, NetworkConnectionToClient sender = null)
    {
        if (!_connectedNameDic.ContainsKey(sender))
        {
            ChatUser player = sender.identity.GetComponent<ChatUser>();
            string palyerName = player.PlayerName;
            _connectedNameDic.Add(sender, palyerName);
        }

        if (!string.IsNullOrWhiteSpace(message))
        {
            string senderName = _connectedNameDic[sender];
            OnTestMessage(senderName, message);
        }
    ...
    //}
    [ClientRpc]
    void OnTestMessage(string senderName,string msg)
    {
        string formatedMsg;

        if (senderName == _localPlayerName)
            formatedMsg = $"{senderName}:{msg}";
        else
            formatedMsg = $"{senderName}:{msg}";
